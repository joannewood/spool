...
# machine. Auto-populated by `host-helper/configure_apps.py` (run by
# setup.sh, or standalone) from whatever's actually installed — edit by
# hand instead if you'd rather (find the exact bundle name via `ls
# ~/Applications /Applications` or `PlistBuddy -c "Print :CFBundleName"
# <bundle>/Contents/Info.plist`), just re-run install.sh afterwards either way.
