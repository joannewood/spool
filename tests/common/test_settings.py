...
def test_get_app_settings_reflects_updates(conn):
    with conn.cursor() as cur:
        cur.execute(
            "UPDATE app_settings SET rescan_enabled = false, rescan_interval_seconds = 60, "
            "auto_accept_archives = true WHERE id = 1"
        )
    assert get_app_settings(conn) == {
        "rescan_enabled": False,
        "rescan_interval_seconds": 60,
        "auto_accept_archives": True,
    }
